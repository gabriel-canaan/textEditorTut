...
class colorGenerator extends Random {
  ...
  color() {
    let r = super.randomInt(0, 256)
    let g = super.randomInt(0, 256)
    let b = super.randomInt(0, 256)
    if (this.type === 'hex') {
...
export async function getRandomPoem() {
  try {
    ...
    let poem = responce.data.allPoems[0]
    return poem.text
  } catch (error) {
...
// fetch(shakespeareApi, options()).then(responce => responce.json()).then(json => {
//   console.log(json)
// })
